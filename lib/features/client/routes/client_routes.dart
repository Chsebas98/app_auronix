    ...
    return [
      ShellRoute(
        ...
        routes: [
          ...
          GoRoute(
            name: 'messagesClient',
            path: ClientRoutesPath.messages,
            builder: (context, state) {
...
class ClientShell extends StatefulWidget {
  ...
  const ClientShell({required this.child, Key? key}) : super(key: key);
...
class _ClientShellState extends State<ClientShell> with WidgetsBindingObserver {
            ...
            builder: (context, navState) {
              ...
              return DecoratedBox(
                ...
                child: BottomNavigationHome(
                  role: Roles.rolUser, // Siempre rolUser porque es ClientRoutes
  ...
  void _syncIndexWithRoute(BuildContext context) {
    ...
    // Mapeo específico para Cliente/Pasajero
    if (location.startsWith(ClientRoutesPath.home)) {
      newIndex = 0; // Home
    } else if (location.startsWith(ClientRoutesPath.messages)) {
      newIndex = 1; // Mensajes
    } else if (location.startsWith(ClientRoutesPath.saveTrips)) {

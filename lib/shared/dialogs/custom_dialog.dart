...
class CustomDialog {
  ...
  Widget get _buildDialog => PopScope(
    onPopInvokedWithResult: (didPop, result) async {
      // Si el pop ya ocurrió, no hagas nada
      if (didPop) return;

      // Reutiliza tu lógica anterior
      final allow = await _onWillPop();
      if (!allow) return;

      _dismissType = DismissType.btnCancel;
      dismiss.call(); // o Navigator.of(context, rootNavigator: true).pop();
    },
  ...
  Widget _getDialogWidget({required Widget child}) {
    ...
    return KeyboardListener(
      ...
      onKeyEvent: (KeyEvent event) {
        // Dispara solo al presionar (no en key up)
...
///Defines the header of [CustomDialog]
enum DialogType {
...
///Defines dismiss type of [CustomDialog]
enum DismissType {
